...
function CameraSetup({ modelBox }: { modelBox: THREE.Box3 | null }) {
  ...
  useEffect(() => {
    ...
    const distance = (maxDim / 2 / Math.tan(fov / 2)) * 0.1; // Much bigger (smaller multiplier = bigger model on screen)
      ...
      >
        ...
        {enableControls && (
          <OrbitControls
            ...
            minDistance={
              modelBox
                ? Math.max(...modelBox.getSize(new THREE.Vector3())) * 1.2
                : 2
            }
            maxDistance={
              modelBox
                ? Math.max(...modelBox.getSize(new THREE.Vector3())) * 4
                : 10
            }
          />
        )}
